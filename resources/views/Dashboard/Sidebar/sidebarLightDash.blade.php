    ...
    <aside class="sidebar-wrapper" data-simplebar="true">
        <div class="sidebar-header">
            ...
            <div>
                <h4 class="logo-text">Freelancer Site</h4>
            </div>
        ...
        <ul class="metismenu" id="menu">
            <li>
                ...
                <ul>
                    <li> <a href="{{ route($guard . '.lightDash') }}"><i class="bi bi-circle"></i>Light Dashboard</a>
                    </li>
                    <li> <a href="{{ route($guard . '.darkDash') }}"><i class="bi bi-circle"></i>Dark Dashboard</a>
                    </li>
                </ul>
            </li>
            ...
            @if (auth('admin')->check())
                <li class="menu-label">
                    {{ auth('admin')->user()->hasRole('super') ? 'Super Admin' : 'Admin' }}
                </li>
            ...
            @elseif (auth('freelancer')->check())
                <li class="menu-label">
                    {{ auth('freelancer')->user()->hasRole('super') ? 'Super Freelancer' : 'Freelancer' }}
                </li>
            ...
            @elseif (auth('web')->check())
                <li class="menu-label">
                    {{ auth('web')->user()->hasRole('super') ? 'Super Client' : 'Client' }}
                </li>
            ...
            <li>
                ...
                <ul>
                    <li> <a href="{{ route($guard . '.logout') }}" target="_blank"><i class="bi bi-circle"></i>Sign
                            In</a>
                    </li>
                    <li> <a href="{{ route($guard . '.register') }}" target="_blank"><i class="bi bi-circle"></i>Sign
                            Up</a>
                    </li>
                    <li> <a href="{{ route($guard . '.forgetPass') }}" target="_blank"><i
                                class="bi bi-circle"></i>Forgot
                            Password</a>
                    </li>
                    <li> <a href="{{ route($guard . '.confirmPass') }}" target="_blank"><i
                                class="bi bi-circle"></i>Reset
                            Password</a>
                    </li>
                </ul>
    ...
    {{-- @php
                // Define current guard from like (defaults or session).
                $guards = ['web', 'freelancer', 'admin'];
                $currentUser = null;
                $currentGuard = null;

                foreach ($guards as $guard) {
                    if (auth($guard)->check()) {
                        // Store[web, freelancer, admin]
                        $currentUser = auth($guard)->user();

                        // Store[web, freelancer, admin]
                        $currentGuard = $guard;
                        // Stoped on first activate guard.
                        break;
                    }
                }
            @endphp
            ...
            @if ($currentUser)
                @php
                    // Define label name based on guard and role.
                    $roleLabel = $currentUser->hasRole('super') ? 'Super ' : '';
                    $guardLabel = match ($currentGuard) {
                        'admin' => 'Admin',
                        'freelancer' => 'Freelancer',
                        'web' => 'Client',
                        // ucfirst (Return Capital latter for first charachter of word).
                        default => ucfirst($currentGuard),
                    };
                @endphp

                <li class="menu-label">{{ $roleLabel }}{{ $guardLabel }}</li>
            @endif --}}
